    ...
    for (i = 0; i < height; i++)
    {
        for(j = 0; j < width; j++)
        {
            ...
            else if (maze[i][j] == PATH)
            {
                int dotCount = 0;
                int nextToStart = 0;
                if (i + 1 < height && (maze[i + 1][j] == PATH || (nextToStart |= maze[i + 1][j] == START) || maze[i + 1][j] == END))
                    dotCount++;
                if (i - 1 > 0 && (maze[i - 1][j] == PATH || (nextToStart |= maze[i - 1][j] == START) || maze[i - 1][j] == END))
                    dotCount++;
                if (j + 1 < width && (maze[i][j+1] == PATH || (nextToStart |= maze[i][j+1] == START) || maze[i][j+1] == END))
                    dotCount++;
                if (j - 1 > 0 && (maze[i][j-1] == PATH || (nextToStart |= maze[i][j-1] == START) || maze[i][j-1] == END))
                    dotCount++;
                if (dotCount == 1 && nextToStart)
                    continue;
                if (dotCount != 2)
                {
                    if (print)
